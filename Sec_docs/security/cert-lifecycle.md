...
## Issuance & Archival
- **Initial issuance** is handled via `fabric-dev-network/organizations/fabric-ca/registerEnroll.sh` (sourced by `network.sh ... -ca`). The script enrolls CA admins, registers peer/admin/client/auditor identities, and places MSP/TLS material under `organizations/*`.
- **Archival requirements:** After provisioning, each MSP is exported via `tar --exclude *_sk -czf <org>-msp.tar.gz organizations/.../msp` and stored in `infra/msp-archives/`. SHA256 hashes accompany the archives (see `infra/README.md`) so we can prove integrity without private keys leaving the repo.
- **Evidence trail:** All issuance commands and archive hashes are logged in `fabric-dev-network/infra/README.md` for the MSP hardening step. Any future issuance (e.g., onboarding a new peer) must append to that log.
- **Automation tie-ins:** `scripts/vault/seed_ca_material.sh` keeps Vault secrets synchronized with on-disk PEMs after CA reinitialization; run it whenever `compose/compose-ca.yaml` is recreated or Vault tokens rotate.
...
## Revocation Workflow
**Triggers:** key compromise, credential loss, offboarding, CA rotation rehearsals, or regulator-mandated kill switch events. Any such trigger must be recorded in the incident tracker and linked back to this workflow.
