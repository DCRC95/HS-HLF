...
# Prefer project-local Fabric binaries: fabric-samples/bin
export PATH=${ROOTDIR}/fabric-samples/bin:${ROOTDIR}/../bin:${PWD}/../bin:$PATH
...
function checkPrereqs() {
  ## Check if your have the peer binary available.
  peer version > /dev/null 2>&1 || {
    errorln "Peer binary not found in PATH.."
    errorln
    errorln "Ensure fabric-samples/bin is present and PATH is set by network.sh."
    exit 1
  }
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ] || [ "$CRYPTO" == "fabric-ca" ]; then
    infoln "Generating certificates using Fabric CA"
    ${CONTAINER_CLI_COMPOSE} -f compose/$COMPOSE_FILE_CA up -d 2>&1

    . organizations/fabric-ca/registerEnroll.sh

    # Make sure CA files have been created (check BankA CA as representative)
    while :
    do
      if [ ! -f "organizations/fabric-ca/banka/ca-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
      sleep 1
      set -x
      fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7054 --caname ca-banka --tls.certfiles "${PWD}/organizations/fabric-ca/banka/ca-cert.pem"
      res=$?
    { set +x; } 2>/dev/null
    rc=$res  # Update rc
    COUNTER=$((COUNTER + 1))
    ...
    infoln "Creating BankA Identities"
    create_banka

    infoln "Creating BankB Identities"
    create_bankb

    infoln "Creating ConsortiumOps Identities"
    create_consortiumops

    infoln "Creating RegulatorObserver Identities"
  ...
  infoln "Generating CCP files for all peer organizations"
...
# call the script to create the channel, join the peers of all four peer orgs,
...
function networkDown() {
  ...
  COMPOSE_FILES="${COMPOSE_BASE_FILES} ${COMPOSE_COUCH_FILES} ${COMPOSE_CA_FILES}"
    ...
    # stop org3 containers also in addition to the main peer orgs, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.banka.example.com docker_peer0.bankb.example.com docker_peer0.consortiumops.example.com docker_peer0.regulatorobserver.example.com 2>/dev/null || true
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/banka/msp organizations/fabric-ca/banka/tls-cert.pem organizations/fabric-ca/banka/ca-cert.pem organizations/fabric-ca/banka/IssuerPublicKey organizations/fabric-ca/banka/IssuerRevocationPublicKey organizations/fabric-ca/banka/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/bankb/msp organizations/fabric-ca/bankb/tls-cert.pem organizations/fabric-ca/bankb/ca-cert.pem organizations/fabric-ca/bankb/IssuerPublicKey organizations/fabric-ca/bankb/IssuerRevocationPublicKey organizations/fabric-ca/bankb/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/consortiumops/msp organizations/fabric-ca/consortiumops/tls-cert.pem organizations/fabric-ca/consortiumops/ca-cert.pem organizations/fabric-ca/consortiumops/IssuerPublicKey organizations/fabric-ca/consortiumops/IssuerRevocationPublicKey organizations/fabric-ca/consortiumops/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/regulatorobserver/msp organizations/fabric-ca/regulatorobserver/tls-cert.pem organizations/fabric-ca/regulatorobserver/ca-cert.pem organizations/fabric-ca/regulatorobserver/IssuerPublicKey organizations/fabric-ca/regulatorobserver/IssuerRevocationPublicKey organizations/fabric-ca/regulatorobserver/fabric-ca-server.db'
